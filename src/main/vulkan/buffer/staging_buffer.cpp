...
ao::vulkan::StagingBuffer::StagingBuffer(std::weak_ptr<Device> device, vk::CommandBufferUsageFlags cmd_usage, bool memory_barrier)
    : ao::vulkan::Buffer(device), cmd_usage(cmd_usage), memory_barrier(memory_barrier), fence(ao::core::shared(this->device)->logical) {}
    ...
    this->command_buffer.begin(vk::CommandBufferBeginInfo(this->cmd_usage));
    {
        ...
        if (this->memory_barrier) {
            vk::BufferMemoryBarrier barrier(
                vk::AccessFlagBits::eTransferWrite, vk::AccessFlags(), _device->queues->at(vk::to_string(vk::QueueFlagBits::eTransfer)).family_index,
                _device->queues->at(vk::to_string(vk::QueueFlagBits::eGraphics)).family_index, this->device_buffer->buffer());
