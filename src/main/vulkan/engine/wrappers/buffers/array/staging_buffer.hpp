...
namespace ao::vulkan {
	...
	public:
		...
		StagingDynamicArrayBuffer(size_t const _count, std::weak_ptr<Device> device, vk::CommandBufferUsageFlags const _usage = vk::CommandBufferUsageFlagBits::eSimultaneousUse, bool const _memoryBarrier = false);
		...
		StagingDynamicArrayBuffer<T>* init(vk::DeviceSize const size, std::optional<vk::BufferUsageFlags> const usageFlags = std::nullopt);
	...
	template<class T>
	DynamicArrayBuffer<T>* StagingDynamicArrayBuffer<T>::updateFragment(std::size_t const index, T const* data) {
		if (!this->hasBuffer()) {
			throw core::Exception("Buffer hasn't been initialized");
		}
	...
	template<class T>
	vk::Buffer const& StagingDynamicArrayBuffer<T>::buffer() const {
	...
	public:
		...
		/// <param name="_usage">Usage</param>
		/// <param name="_memoryBarrier">Bind memory barrier on transfer</param>
		StagingArrayBuffer(std::weak_ptr<Device> device, vk::CommandBufferUsageFlags const _usage = vk::CommandBufferUsageFlagBits::eSimultaneousUse, bool const _memoryBarrier = false);
		...
		/// <param name="usageFlags">Usage flags</param>
		/// <returns>This</returns>
		StagingArrayBuffer<T, N>* init(vk::DeviceSize const size, std::optional<vk::BufferUsageFlags> const usageFlags = std::nullopt);
	...
	template<class T, size_t N>
	ArrayBuffer<T, N>* StagingArrayBuffer<T, N>::updateFragment(std::size_t const index, T const* data) {
		if (!this->hasBuffer()) {
			throw core::Exception("Buffer hasn't been initialized");
		}
	...
	template<class T, size_t N>
	vk::Buffer const& StagingArrayBuffer<T, N>::buffer() const {
		return StagingBuffer::buffer();
	}
