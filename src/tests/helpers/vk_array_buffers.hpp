...
namespace ao {
	namespace test {
		...
		void * TestStagingDynamicArrayBuffer<T>::getMapper() {
			if (auto host = static_cast<TestBasicDynamicArrayBuffer<T>*>(this->hostBuffer.get())) {
		...
		TestStagingArrayBuffer<T, N>::TestStagingArrayBuffer(std::weak_ptr<vulkan::Device> device) :
			vulkan::StagingArrayBuffer<T, N>(device, vk::CommandBufferUsageFlagBits::eOneTimeSubmit),
			vulkan::ArrayBuffer<T, N>(device) {}

		template<class T, size_t N>
		void * TestStagingArrayBuffer<T, N>::getMapper() {
			if (auto host = static_cast<TestBasicArrayBuffer<T, N>*>(this->hostBuffer.get())) {
				return host->getMapper();
			}
			throw core::Exception("Fail to get mapper");
		}
