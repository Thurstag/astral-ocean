...
namespace ao::vulkan {
	...
	public:
		...
		/// <returns>This</returns>
		BasicTupleBuffer<T...>* init(vk::BufferUsageFlags const& usageFlags, vk::SharingMode const sharingMode, vk::MemoryPropertyFlags const memoryFlags, std::initializer_list<vk::DeviceSize> const& sizes);
	...
	template<class ...T>
	BasicTupleBuffer<T...>* BasicTupleBuffer<T...>::init(vk::BufferUsageFlags const& usageFlags, vk::SharingMode const sharingMode, vk::MemoryPropertyFlags const memoryFlags, std::initializer_list<vk::DeviceSize> const& sizes) {
		if (this->hasBuffer()) {
			this->free();
		}
		auto _device = ao::core::shared(this->device);
	...
	template<class ...T>
	TupleBuffer<T...>* BasicTupleBuffer<T...>::updateFragment(std::size_t const index, void* const data) {
		if (!this->hasBuffer()) {
			throw core::Exception("Buffer hasn't been initialized");
		}
	...
	template<class ...T>
	vk::DeviceSize BasicTupleBuffer<T...>::offset(size_t const index) const {
		if (!this->hasMapper) {
